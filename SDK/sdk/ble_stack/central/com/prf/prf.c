...
#if (BLE_CTS_SERVER)
extern const struct prf_task_cbs* ctss_prf_itf_get(void);
#endif // (BLE_FFF0_SERVER)
 ...
 ****************************************************************************************
 */
struct prf_env_tag prf_env[BLE_CONNECTION_MAX];
...
void prf_init(bool reset)
{
    ...
    if(!reset)
    {
        ...
        for(idx=0;idx<BLE_CONNECTION_MAX;idx++)
        {
            ...
            {
                prf_env[idx].prf[i].env  = NULL;
                prf_env[idx].prf[i].task = TASK_GAPC + i +1;
                prf_env[idx].prf[i].id   = TASK_ID_INVALID;

                // Initialize Task Descriptor
                prf_env[idx].prf[i].desc.default_handler = NULL;
                prf_env[idx].prf[i].desc.state           = NULL;
                prf_env[idx].prf[i].desc.idx_max         = 0;

                prf_env[idx].prf[i].desc.state_max       = 0;
                prf_env[idx].prf[i].desc.state_handler   = NULL;

                ke_task_create(prf_env[idx].prf[i].task, &(prf_env[idx].prf[i].desc));
            }
        }
    }
    else
    {
        ...
        for(idx=0;idx<BLE_CONNECTION_MAX;idx++)
        {
            ...
            {
                // Get Profile API
                const struct prf_task_cbs * cbs = prf_itf_get(prf_env[idx].prf[i].id);
                if(cbs != NULL)
                {
                    // request to destroy profile
                    cbs->destroy(&(prf_env[idx].prf[i]));
                }
                // unregister profile
                prf_env[idx].prf[i].id   = TASK_ID_INVALID;
                prf_env[idx].prf[i].desc.default_handler = NULL;
                ...
                ke_task_msg_flush(KE_TYPE_GET(prf_env[idx].prf[i].task));
            }
        }
    }
}
...
uint8_t prf_add_profile(struct gapm_profile_task_add_cmd * params, ke_task_id_t* prf_task)
{
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        for(i = 0; i < BLE_NB_PROFILES ; i++)
        {
            if(prf_env[idx].prf[i].id == params->prf_task_id)
            {
                status = GAP_ERR_NOT_SUPPORTED;
                UART_PRINTF("GAP_ERR_NOT_SUPPORTED i=%x, prf_task_id=%x, app_task=%x\n",i,params->prf_task_id,params->app_task);
                break;
            }
        }
    }
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        ...
        for(i = 0; i < BLE_NB_PROFILES ; i++)
        {
            ...
            if(prf_env[idx].prf[i].id == TASK_ID_INVALID)
            {
                // initialize profile
                status = cbs->init(&(prf_env[idx].prf[i]), &(params->start_hdl), params->app_task, params->sec_lvl, params->param);

                // initialization succeed
                if(status == GAP_ERR_NO_ERROR)
                {
                    // register profile
                    prf_env[idx].prf[i].id = params->prf_task_id;
                    *prf_task = prf_env[idx].prf[i].task;
                    UART_PRINTF("prf_add_profile i = %d*prf_task :0x%04x\r\n",i,*prf_task);
                    UART_PRINTF("default_handler = 0x%x\r\n",prf_env[idx].prf[i].desc.default_handler);
                    UART_PRINTF("prf_env.prf[i].id=0x%x\r\n",prf_env[idx].prf[i].id);
                }
                break;
            }
        }
...
void prf_create(uint8_t conidx)
{
    ...
    {
        // Get Profile API
        const struct prf_task_cbs * cbs = prf_itf_get(prf_env[conidx].prf[i].id);
        if(cbs != NULL)
        {
            // call create callback
            cbs->create(&(prf_env[conidx].prf[i]), conidx);
        }
    }
}
...
void prf_cleanup(uint8_t conidx, uint8_t reason)
{
    uint8_t i;
    /* simple connection creation handler, nothing to do. */
    UART_PRINTF("prf_cleanup,conidx=%x\n",conidx);
    // execute create function of each profiles
    for(i = 0; i < BLE_NB_PROFILES ; i++)
    {
        // Get Profile API
        ...
        if(cbs != NULL)
        {
            ...
            cbs->cleanup(&(prf_env[conidx].prf[i]), conidx, reason);
            UART_PRINTF("(prf_env[%x].prf[%x].id=%x\n",conidx,i,prf_env[conidx].prf[i].id);
            //prf_env[idx].prf[i].id
...
prf_env_t* prf_env_get(uint16_t prf_id)
{
    prf_env_t* env = NULL;
    uint8_t i;
    uint8_t idx = KE_IDX_GET(prf_id);
    // find if profile present in profile tasks
    for(i = 0; i < BLE_NB_PROFILES ; i++)
    {
        // check if profile identifier is known
        if(prf_env[idx].prf[i].id == prf_id)
        {
            env = prf_env[idx].prf[i].env;
            break;
        }
	}
...
struct prf_env_t * prf_env_get_from_uuid(uint8_t conidx,uint8_t uuid_len,const uint8_t *uuid)
{
    ...
    for(i = 0; i < BLE_NB_PROFILES ; i++)
    {	
		//UART_PRINTF("prf_env.prf[%d].id = 0x%x \r\n",i,prf_env[conidx].prf[i].id);
		if(prf_env[conidx].prf[i].role == PRF_CLIENT)
		{
			sdp_env = (struct sdp_env_tag *) prf_env[conidx].prf[i].env;
            ...
            {           
                ...
                for(j = 0;j < sdp_env->prf_db_env->sdp_cont->chars_nb;j++)
                {
                    ...
                    {
                        env = (struct prf_env_t*) prf_env[conidx].prf[i].env;
...
ke_task_id_t prf_get_id_from_task(ke_msg_id_t task)
{
    ...
    {
        // check if profile identifier is known
        if(prf_env[idx].prf[i].task == task)
        {
            id = prf_env[idx].prf[i].id;
            break;
        }
    }
...
ke_task_id_t prf_get_task_from_id(ke_msg_id_t id)
{
    ...
    {
        // check if profile identifier is known
        if(KE_TYPE_GET(prf_env[idx].prf[i].id) == id)
        {
            task = prf_env[idx].prf[i].task;
            break;
        }
    }
    if(task == TASK_NONE)
        UART_PRINTF("prf_env.prf[i].id=%x,id=%x\n",prf_env[idx].prf[i].id,id);
