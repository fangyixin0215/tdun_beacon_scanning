/*************************************************************
 * @file        driver_i2s.h
 * @brief       Header file of driver_i2s.c
 * @author      GuWenFu
 * @version     V1.0
 * @date        2016-09-29
 * @par
...
typedef struct
{
    uint8_t SAMPLE_ALIGN data_buff[AUDIO_BUFF_LEN];
...
void i2s_cfg(uint32_t freq, uint16_t datawidth);
void i2s_open(AUDIO_CTRL_BLK *ablk);
void i2s_close(void);
void i2s_isr(void);
void i2s_rx_enable(uint8_t enable);
void i2s_volume_mute(uint8_t enable);
void i2s_volume_adjust(uint8_t volume);
void i2s_rx_volume_adjust(uint8_t volume);
void I2S_Test(unsigned char role);
