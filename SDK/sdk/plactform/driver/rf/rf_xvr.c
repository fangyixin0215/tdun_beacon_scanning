...
void kmod_calibration(void)
{
    ...
    value_kcal_result =  ((256*250/value)&0x1ff)+2;
...
#define __CHIP_G__      0x19C20D30
#define __CHIP_D__      0x18620A30
void init_analog_xvr_reg(void)
{
    ...
    if(reg_tmp == __CHIP_G__)
    {
        mHWreg_Assign_XVR_Regsiter(04, 0x5F93FF30);// 0x5F915727  1017
        XVR_ANALOG_REG_BAK[0x04] = 0x5F93FF30;

        mHWreg_Assign_XVR_Regsiter(05, 0x48287500);//0x48295540);// 0x48285540
        XVR_ANALOG_REG_BAK[0x05] = 0x48287500;

        mHWreg_Assign_XVR_Regsiter(06, 0xE1DF0C00);   // for adc
        XVR_ANALOG_REG_BAK[0x06] = 0xE1DF0C00;//
    }
    else
    {
	...
	mHWreg_Assign_XVR_Regsiter(07, 0x00103fc7);//0x0000Bfc7);  //0x0010Bfc7
	XVR_ANALOG_REG_BAK[0x07] = 0x00103fc7;//0x0000Bfc7; //0x00103fc7
    ...
    if(reg_tmp == __CHIP_G__)
    {
        mHWreg_Assign_XVR_Regsiter(0B, 0xD3DD241C);///buck output 1.4v///0xD1FD260C//0xd1Fd240C); //0xD3DD240C  71FD240C
        XVR_ANALOG_REG_BAK[0x0B] = 0xD3DD241C;
    }
    else
    {
...
void initial_xver_BK3435V2_openLoop(void)
{
    ...
    mHWreg_Assign_XVR_Regsiter(2A, 0x0e0f3D6c);
...
///晶体频偏调整
///cal_data默认为0x0,最大为0x0f,值越大，频率越大。
void xtal_cal_set(uint8_t cal_data)
{
    if(cal_data>0x0f)
        cal_data=0x0f;
    XVR_ANALOG_REG_BAK[7] &= (~(0x0f<<14));
    XVR_ANALOG_REG_BAK[7] |= (cal_data<<14);
    XVR_REG07 = XVR_ANALOG_REG_BAK[7] ;
}
...
{
    UART_PRINTF("%s\r\n", __func__);
    XVR_ANALOG_REG_BAK[0xB] &= ~(0x1 << 21);
    XVR_REG0B = XVR_ANALOG_REG_BAK[0xB];
}
